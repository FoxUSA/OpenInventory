...
<v-app dark>
  ...
  <v-navigation-drawer v-model="drawer" fixed clipped app :width="150">
    ...
    <v-subheader class="caption mt-3 grey--text text--darken-2 fade-out-legal">
      &#169; 2019 Jacob Liscom <br /> Version: {{version}}
    </v-subheader>
  </v-navigation-drawer>
  ...
  <v-toolbar color="blue" dense fixed clipped-left app flat>
    <v-toolbar-title v-ripple style="cursor: pointer; overflow: visible;">
      <span class="title" @click.stop="drawer = !drawer">OpenInventory</span>
    </v-toolbar-title>
    ...
    <v-toolbar-items >
      <v-btn flat class="hidden-xs-only" @click.stop="search" style="min-width:0">
        <!-- FIXME no way to search on mobile except to rotate. Maybe just hide search button and use enter to search? -->
        Search
      </v-btn>
      <v-btn flat @click.stop="scanner=true" style="min-width:0">
        Scan
      </v-btn>
    </v-toolbar-items>
  </v-toolbar>
  ...
  <Scanner :scanner.sync="scanner" :callback="scan" />
  <!-- <Scanner v-model="scanner" :callback="search" /> -->
</v-app>
</template>
