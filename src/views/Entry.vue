...
<v-container grid-list-lg fluid>
              ...
              <v-form class="flex xs12">
                <v-layout wrap>
                  <v-flex :class="field.size ? `xs${field.size}` : 'xs12 sm6 xl4'" v-for="field in group.fields" :key="field.name">
                    ...
                    <!-- String or path -->
                    <v-text-field :color="field.color ? field.color :'white'" v-if="field.type=='string'|| field.type=='path'" v-model="item[field.name]" :label="field.displayName" :disabled="field.disabled"></v-text-field>
                    ...
                    <template v-if="field.type=='list'">
                      ...
                      <v-btn depressed :color="field.color ? field.color :'primary'" v-if="field.listType=='upc'" @click.stop="setScannerCallback(field)">Scan</v-btn>
                    </template>
  ...
  <Scanner :scanner.sync="tempState.toggleScanner" :callback="tempState.scannerCallback" />
...
export default {
  ...
  methods: {
    ...
    /**
     * Creates and set a scanner callback. Activates scanner
     * @param  {[type]} field - field you want the scanner to modify
     */
    setScannerCallback: function (field) {
      this.tempState.toggleScanner = true

      // If field values is blank we need to set it to an array
      if (!this.item[field.name]) { this.$set(this.item, field.name, []) }

      this.tempState.scannerCallback = (code) => {
        this.tempState.toggleScanner = false
        if (!this.item[field.name].includes(code)) { // Make sure the code is not in the array
          this.item[field.name].push(code) // othwerwise add it
        }
      }
    },
    // activateScanner: function (field) {
    //   this.tempState.toggleScanner = true
    ...
    //       let fieldValue = this.item[field.name]
    //       let code = result.codeResult.code
    //
    //       if (fieldValue[fieldValue.length - 1] !== code) { fieldValue.push(code) } // Make sure we dont add this code multiple times
    //
    //       this.tempState.toggleScanner = false
    //       Quagga.offDetected()
    //       Quagga.stop()
