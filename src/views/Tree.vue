...
<v-container grid-list-lg fluid>
  <v-layout>
    <v-flex xs6>
      <h4 class="display-1">Item tree</h4>
    </v-flex>
  </v-layout>
  <!-- TODO breadcrumbs -->
...
export default {
  ...
  methods: {
    ...
    loadData: function (skip = 0, limit = 100) {
      let prefix = (this.$route.query.prefix || '').toLowerCase().split('/')
      ...
      let buildTree = () => {
        ...
        return new Promise((resolve, reject) => {
          ConfigService.getConfigObject().then((config) => {
            ...
            let pathFieldName = this.pathField.charAt(0).toUpperCase() + this.pathField.slice(1)
            let loopFunction = () => {
              DatabaseService().getAllItems({ skip: i, limit: RECORD_TO_PULL }).then((data) => {
                ...
                data.rows.forEach((item) => {
                  let path = (item[this.pathField] || 'none').toLowerCase()
      ...
      // Execute
      return new Promise((resolve, reject) => {
        if (this.tree._items.length) { return resolve(loadPrefix(prefix)) }// If tree is already set Ether prefixes or

        buildTree().then(() => { // Build a cache
          return resolve(loadPrefix(prefix))
        })// .catch(reject)
      })
    }
  }
